...
function infected(s) {
	// create two variables: total, infected
	let total = 0;
	let infected = 0;
	// get strings in between 'X'
	const continents = s.split('X');
	// for every string 
	for (let continent of continents) {
		// add the number of chars to total
		total += continent.length;
		// if there is a '1'
		if (continent.includes('1')) {
			//  add this number to infected as well
			infected += continent.length;
		}
	}
	// we cannot divide by zero
	if (total === 0) return 0
	// return the percentage calculation
