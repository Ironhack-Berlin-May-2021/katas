...
function cakes(recipe, available) {
	...
	for (let ingredient in recipe) {
		// if this ingredient is not in the available object then we can return 0
		if (!available[ingredient]) return 0
		// add the amounts (available / recipe) to the amounts array
		amounts.push(Math.floor(available[ingredient] / recipe[ingredient]))
	}
	// get the lowest number from the amounts array
